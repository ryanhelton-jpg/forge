...
app.get('/api/swarm/:runId/status', (req, res) => {
    ...
    if (swarm.status === 'complete' && swarm.result) {
        response.result = swarm.result;
    }
    if (swarm.status === 'error' && swarm.error) {
        response.error = swarm.error;
    }
    res.json(response);
});
...
function getOrCreateDefaultGenome() {
    const store = getGenomeStore();
...
function syncGenomeToPersona(genome) {
...
app.delete('/api/genome/trait/:id', (req, res) => {
    try {
        ...
        if (error.message === 'Trait not found') {
