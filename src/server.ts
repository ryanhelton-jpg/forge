...
app.post('/api/chat/stream', async (req, res) => {
  ...
  try {
    ...
    let evolutionProposals: Array<{ type: string; description: string; reason: string; confidence: number }> = [];
    try {
      const store = getGenomeStore();
      const genome = getOrCreateDefaultGenome();
      store.recordInteraction(genome.id, {
    ...
    const sidebarRefresh = toolCalls?.some((tc: { name: string }) => modifyingTools.includes(tc.name)) || false;
    ...
    sendEvent('done', {
      ...
      evolutionProposals,
      sidebarRefresh,
      toolsUsed: toolCalls?.map((tc: { name: string }) => tc.name) || [],
    });
...
async function runSwarmAsync(
  runId: string, 
  goal: string | undefined, 
  plan: SwarmPlan | undefined, 
  ...
  const orchestrator = new Orchestrator({
    ...
    maxTotalTurns: 30,
    onPlanReady: (plan) => {
      console.log(`[planner] plan ready: ${plan.tasks.length} tasks`);
      const swarm = activeSwarms.get(runId);
      if (swarm) {
    ...
    onAgentStart: (roleId, task) => {
      ...
      if (!agentsUsed.includes(roleId)) agentsUsed.push(roleId);
      const swarm = activeSwarms.get(runId);
      if (swarm) {
  ...
  try {
    let result;
    if (plan) {
      result = await orchestrator.executePlan(plan);
    } else {
      result = await orchestrator.execute(sanitizeInput(goal || ''));
    }

    executionLog.updateSwarmInfo(runId, { protocol: plan?.protocol || protocol || 'auto', tasksCount: result.tasks?.length || 0, agentsUsed });
    ...
    if (swarm) {
      ...
      // Store result in swarm for retrieval
      (swarm as any).result = result;
    ...
    swarmPersistence.complete(runId, {
      ...
      tasks: result.tasks?.map((t: SwarmTask) => ({ 
        id: t.id, 
    ...
    if (swarm) {
      swarm.status = 'error';
      (swarm as any).error = error instanceof Error ? error.message : 'Unknown error';
...
app.get('/api/swarm/:runId/status', (req, res) => {
  ...
  if (swarm.status === 'complete' && (swarm as any).result) {
    response.result = (swarm as any).result;
  }
  if (swarm.status === 'error' && (swarm as any).error) {
    response.error = (swarm as any).error;
  }
