...
export class Orchestrator {
   ...
   */
  async executePlan(plan: SwarmPlan): Promise<Omit<SwarmResult, 'timing'>> {
    const startedAt = Date.now();
   ...
   */
  private async planTask(goal: string): Promise<SwarmPlan> {
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          tasks: (parsed.tasks || []).map((t: Partial<SwarmTask>, i: number) => ({
            id: t.id || `t${i + 1}`,
   ...
   */
  private async executeSequential(plan: SwarmPlan): Promise<Omit<SwarmResult, 'timing'>> {
    const completedTasks = new Map<string, SwarmTask>();
    ...
    for (const task of sortedTasks) {
      ...
      const depsComplete = (task.dependencies || []).every(
        depId => completedTasks.get(depId)?.status === 'complete'
      );
   ...
   */
  private async executeParallel(plan: SwarmPlan): Promise<Omit<SwarmResult, 'timing'>> {
   ...
   */
  private async executeDebate(plan: SwarmPlan): Promise<Omit<SwarmResult, 'timing'>> {
    const maxRounds = 3;
   ...
   */
  private buildTaskContext(task: SwarmTask, plan: SwarmPlan): string {
    const parts: string[] = [];
   ...
   */
  private groupByDependencyLevel(tasks: SwarmTask[]): SwarmTask[][] {
