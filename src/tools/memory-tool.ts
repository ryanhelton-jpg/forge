...
export function createMemoryTool(memory: Memory): Tool {
  return {
    name: 'memory',
    ...
    execute: async (params) => {
      ...
      const category = (params.category as 'preference' | 'fact' | 'context') || 'fact';

      if (action === 'store') {
        if (!text) return 'Error: text required for store action';
      ...
      if (action === 'recall') {
        ...
        if (facts.length === 0) return 'No matching memories found.';
