...
export function hydrateCustomTool(def: CustomToolDef): Tool {
  return {
    name: def.name,
...
IMPORTANT: Keep code simple. No async, no external imports. Just pure computation.`,
  ...
  execute: async (params) => {
    ...
    switch (action) {
      case 'list':
        if (tools.length === 0) return 'No custom tools created yet.';
      ...
      case 'create': {
        ...
        // Parse parameters
        let toolParams: Record<string, ToolParameter> = {};
        if (parametersJson) {
          try {
...
The tool is now available. Try using it!`;
      ...
      case 'delete': {
        ...
        if (idx === -1) return `Tool "${name}" not found`;
      ...
      case 'test': {
        const name = params.name as string;
        const testParamsJson = params.test_params as string;
