...
Last updated: ${config.updatedAt}`;
      ...
      case 'add_trait':
        ...
        if (config.traits.includes(value)) return `Trait "${value}" already exists`;
      ...
      case 'remove_trait':
        ...
        if (traitIdx === -1) return `Trait "${value}" not found`;
      ...
      case 'remove_rule':
        ...
        if (ruleIdx === -1) return `No rule matching "${value}" found`;
