...
export class GenomeStore {
  ...
  create(genome: AgentGenome): AgentGenome {
    ...
    `).run(
      ...
      genome.slug,
      genome.version,
      genome.parentId || null,
      JSON.stringify(genome.lineage),
      JSON.stringify(genome.identity),
      JSON.stringify(genome.tools),
      JSON.stringify(genome.skills),
      JSON.stringify(genome.knowledge),
      JSON.stringify(genome.config),
      genome.fitness ? JSON.stringify(genome.fitness) : null,
      genome.status,
      genome.visibility,
      genome.stats.totalInteractions,
      genome.stats.totalTokens,
      genome.stats.totalCost,
      now,
      now
    );
  ...
  get(id: string): AgentGenome | null {
    const row = this.db.prepare('SELECT * FROM genomes WHERE id = ?').get(id) as any;
    if (!row) return null;
    return this.rowToGenome(row);
  }

  getBySlug(userId: string, slug: string): AgentGenome | null {
    const row = this.db.prepare('SELECT * FROM genomes WHERE user_id = ? AND slug = ?').get(userId, slug) as any;
    if (!row) return null;
    return this.rowToGenome(row);
  }
  ...
  list(userId: string): AgentGenome[] {
    const rows = this.db.prepare('SELECT * FROM genomes WHERE user_id = ? ORDER BY updated_at DESC').all(userId) as any[];
  ...
  removeTrait(genomeId: string, traitId: string): AgentGenome {
    ...
    if (idx === -1) throw new Error('Trait not found');
  ...
  getMutations(genomeId: string, limit = 50): MutationRecord[] {
    return this.db.prepare(`
      SELECT * FROM mutations 
      WHERE genome_id = ? 
      ORDER BY timestamp DESC 
      LIMIT ?
    `).all(genomeId, limit) as MutationRecord[];
  }

  getVersionHistory(genomeId: string): { version: string; timestamp: number; description: string }[] {
    const mutations = this.getMutations(genomeId, 100);
    const versions = new Map<string, { version: string; timestamp: number; description: string }>();
  ...
  rollback(genomeId: string, targetVersion: string): AgentGenome {
    ...
    const toRollback = mutations.filter(m => 
      m.status === 'applied' && this.compareVersions(m.version, targetVersion) > 0
    );
  ...
  recordInteraction(genomeId: string, result: {
    ...
    if (result.userFeedback) {
      ...
      genome.fitness.overall = (
        genome.fitness.metrics.taskCompletion * 0.3 +
  ...
  proposeEvolution(genomeId: string, context: {
    recentFeedback: Array<{ feedback: string; sentiment: 'positive' | 'negative' | 'neutral' }>;
    recentTopics: string[];
  }): Array<{
    type: MutationType;
    description: string;
  ...
  private recordMutation(genomeId: string, version: string, mutation: {
    type: string;
    description: string;
    trigger: { source: string; reason?: string };
    ...
    `).run(
      ...
      JSON.stringify(mutation.trigger),
      JSON.stringify(mutation.diff),
      'applied',
      now,
      now
    );
  ...
  private rowToGenome(row: any): AgentGenome {
    return {
      id: row.id,
      userId: row.user_id,
      name: row.name,
...
// Singleton instance
let _store: GenomeStore | null = null;
